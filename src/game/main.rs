...
fn start(args: &str) -> ProgramState {
    ...
    let auth_db = database::spawn_threads(
        cnf.auth_database_threads,
    ...
    let server_data = {
        ...
        let mut server_data = GameServerData::new(
            server,
            io_loop.channel(),
            cnf,
            db,
    ...
    *SYNC_SERVER.lock().unwrap() = Some(server_data.clone());
    ...
    let _ = {
        ...
        let mut sorted_areas: Vec<(i16, i16)> =
            server_data.areas
                       .values()
