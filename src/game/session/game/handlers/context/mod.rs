...
pub fn handle_game_context_create_request(self_: &mut Session, chunk: &Chunk,
    _: Cursor<Vec<u8>>) -> io::Result<()> {

    let ch = match self_.current_character.as_ref() {
        Some(ch) => ch,
        None => return Ok(())
    };
...
pub fn handle_map_informations_request(self_: &mut Session, chunk: &Chunk,
    _: Cursor<Vec<u8>>) -> io::Result<()> {
...
pub fn handle_game_map_movement_request(self_: &mut Session, chunk: &Chunk,
...
pub fn handle_change_map(self_: &mut Session, chunk: &Chunk,
    mut data: Cursor<Vec<u8>>) -> io::Result<()> {

    let ch = match self_.current_character {
        Some(ref ch) => ch,
        None => return Ok(())
    };
