...
pub struct Server {
    base: shared::server::ServerBase<auth::Session, auth::chunk::ChunkImpl,
    ...
    pub fn game_event(&mut self, evt: SessionEvent) {
        if let SessionEvent::Disconnect(tok) = evt {
            match self.game_session_ids.inv_remove(&tok) {
                Some(id) => self.update_game_server(id, server_status::OFFLINE, String::new(), 0),
                None => (),
           }
        }
